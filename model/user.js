...
userSchema.methods.resetTokenHandler = async function () {
    ...
    const hashedPassword = crypto.createHash("sha256").update(resetPassword).digest("hex");
    const updateFields = { $set: { resetToken: hashedPassword, resetTime: Date.now() + 15 * 60 * 1000 }};
